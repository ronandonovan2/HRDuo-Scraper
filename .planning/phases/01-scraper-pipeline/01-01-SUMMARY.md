...
provides:
  ...
  - "src/config.js — URL, CSS selectors, timeout config (selectors are placeholders pending live page inspection)"
  - "src/scraper.js — scrapeJobs() function using Playwright networkidle wait"
  - "src/index.js — orchestrator with validate, zero-result safety, and jobs.json writer"
  - "data/jobs.json — output file (currently empty array pending HR Duo coming back online)"
...
key-decisions:
  ...
  - "Selectors marked with TODO comments — HR Duo in maintenance during build, needs live DOM inspection"
  - "Description truncated to 200 chars to avoid duplicating full HR Duo content"
